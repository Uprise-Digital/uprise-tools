...
import { CheckCircle2, KeyRound, Loader2, Lock, Mail, ShieldCheck, Sparkles, User } from "lucide-react";
import Image from "next/image";
import Link from "next/link";
import { useRouter, useSearchParams } from "next/navigation";
import { useEffect, useState, Suspense } from "react";
import { toast } from "sonner";
import { acceptInvitationAction, getInvitationDetailsAction } from "@/actions/team.actions";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (inviteId && invitationDetails) {
        ...
        if (res.success) {
          toast.success(`Successfully joined ${invitationDetails.organizationName}!`);
        ...
        if (error) {
          ...
          toast.success("Account created successfully! Let's set up your agency.");
      ...
      <CardContent className="p-8 sm:p-10 flex flex-col items-center text-center space-y-6">
        ...
        <div className="flex flex-col items-center space-y-3">
          ...
          <div className="space-y-1.5 pt-1">
            <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-white">
              {invitationDetails ? "Accept Team Invitation" : "Create Agency Workspace"}
            </h1>
          ...
          <div className="w-full p-4 bg-indigo-950/60 border border-indigo-500/30 rounded-xl text-left flex items-start gap-3">
            ...
            <div className="space-y-1 text-xs">
              ...
              <p className="text-indigo-300/80">
                Assigned Role: <span className="uppercase font-extrabold tracking-wide text-indigo-300">{invitationDetails.role}</span>
              </p>
              <p className="text-slate-400">Target Email: {invitationDetails.email}</p>
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="w-full text-left space-y-4">
          <div className="space-y-1.5">
            <Label htmlFor="name" className="text-xs text-slate-300 font-semibold flex items-center gap-1.5">
          ...
          >
            {loading ? (
              ...
              <span>{invitationDetails ? "Accept Invite & Join Workspace" : "Complete Registration"}</span>
            )}
          </Button>
        </form>
...
export default function SignUpPage() {
  return (
    <Suspense fallback={<div className="text-slate-400 text-center py-12">Loading registration...</div>}>
      <SignupFormContent />
    </Suspense>
  );
}
