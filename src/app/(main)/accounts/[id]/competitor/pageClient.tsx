...
import {
  generateThreatMatrixAction,
  getAutoTargetAction,
} from "@/actions/threat-matrix.actions";
...
export default function CompetitorClient({ account }: { account: any }) {
  ...
  useEffect(() => {
    ...
    async function autoDetectTarget() {
      ...
      try {
        const res = await getAutoTargetAction(
        ...
        <Card className="border-slate-200 shadow-sm overflow-hidden relative min-h-[400px] flex flex-col items-center justify-center text-slate-500">
          ...
          <div className="space-y-1 text-sm text-center">
            <p className="animate-fade-in-up delay-100">
              1. Locating client URL in database...
            </p>
            <p className="animate-fade-in-up delay-300">
              2. Querying live Google SERP for top competitors...
            </p>
            <p className="animate-fade-in-up delay-500">
              3. Bypassing bot protection and scraping landing pages...
            </p>
            <p className="animate-fade-in-up delay-700">
              4. Processing Markdown with Gemini 1.5 Flash...
            </p>
          </div>
        </Card>
      )}
              ...
              <CardContent className="space-y-6">
                <div>
                  ...
                  <div className="flex items-end gap-2">
                    <span
                      className={`text-4xl font-black ${report.analysis.competitor_scoring.client_score_out_of_10 < 6 ? "text-red-500" : "text-amber-500"}`}
                    >
                      {
                        report.analysis.competitor_scoring
                          .client_score_out_of_10
                      }
                    </span>
                    <span className="text-lg text-slate-500 font-bold pb-1">
                      / 10
                    </span>
                  </div>
                </div>
                <div className="border-t border-slate-800 pt-4">
                  <p className="text-xs text-slate-400 mb-1">
                    ...
                    <span className="text-emerald-400 font-medium">
                      {report.analysis.competitor_scoring.market_leader_name}
                    </span>
                  </p>
                  <div className="flex items-end gap-2">
                    ...
                    <span className="text-sm text-slate-500 font-bold pb-1">
                      / 10
                    </span>
                  </div>
                </div>
          ...
          <Card className="border-slate-200 shadow-sm">
            <CardHeader className="border-b border-slate-100 bg-slate-50/50">
              <CardTitle className="text-base font-bold text-slate-800 flex items-center gap-2">
                <TrendingDown className="h-5 w-5 text-red-500" /> Identified
                Conversion Gaps
              </CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              <div className="divide-y divide-slate-100">
                {report.analysis.market_gaps.map((gap: any, i: number) => (
                  <div
                    key={i}
                    className="p-6 grid grid-cols-1 md:grid-cols-12 gap-4 hover:bg-slate-50 transition-colors"
                  >
                    ...
                    <div className="md:col-span-5 space-y-1">
                      <p className="text-xs font-bold text-emerald-600 uppercase tracking-wide">
                        Winning Competitor Strategy
                      </p>
                      <p className="text-sm text-slate-900 font-medium">
                        {gap.competitor_winning_strategy}
                      </p>
                    </div>
                  </div>
                ))}
