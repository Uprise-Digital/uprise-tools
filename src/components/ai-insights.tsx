...
function EfficiencyBadge({score}: { score: string }) {
    ...
    if (isUnder)
        return (
            <span
                className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold bg-red-50 text-red-600 border border-red-200">
...
function PriorityBadge({priority}: { priority: string }) {
    ...
    if (p === "high")
        return <span className="text-[10px] font-bold tracking-wide uppercase text-red-500">High</span>;
    if (p === "medium")
        return <span className="text-[10px] font-bold tracking-wide uppercase text-amber-500">Med</span>;
    return <span className="text-[10px] font-bold tracking-wide uppercase text-slate-400">Low</span>;
}
...
export function AiInsights({
    ...
    const fetchData = async (forceRefresh: boolean, isMounted: boolean = true) => {
        ...
        try {
            ...
            if (isMounted) setError(e.message ?? "Analysis failed. Please try again.");
        } finally {
            if (isMounted) {
    ...
    if (isInitialLoading) {
        return (
            <div
                className="my-6 flex items-center justify-center gap-3 text-sm text-slate-500 py-12 rounded-xl border border-dashed border-slate-200 bg-slate-50/50">
    ...
    if (!insights && !isInitialLoading) {
        return (
            <div className="my-6 py-6 border-t border-slate-200">
                {error && (
                    <div
                        className="mb-3 flex items-center gap-2 text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2">
    ...
    return (
        <div className="my-8 space-y-3 font-sans">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 mb-5 px-1">
                <div>
                    <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                        <Sparkles className="h-4 w-4 text-blue-600"/> Strategic Intelligence
                    </h2>
                    {/* Data Provenance */}
                    <div className="flex items-center gap-1.5 mt-1 text-xs font-medium text-slate-500">
                        <Database className="h-3 w-3"/>
                        Google Ads • {formatShortDate(startDate)} to {formatShortDate(endDate)}
                    </div>
                </div>
                ...
                {generatedAt && (
                    <div className="flex items-center gap-2">
                        {/* Generation Time */}
                        <div
                            className="flex items-center gap-1.5 bg-slate-100 border border-slate-200 px-2.5 py-1.5 rounded-md text-[10px] font-semibold text-slate-500">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm relative overflow-hidden">
                {isRefreshing && <div className="absolute inset-0 bg-white/50 backdrop-blur-[1px] z-10"/>}
                <div className="flex items-start gap-3">
                    <div
                        className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-blue-50 border border-blue-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 relative">
                {isRefreshing && <div className="absolute inset-0 bg-white/50 backdrop-blur-[1px] z-10 rounded-xl"/>}
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <div className="mb-6 rounded-lg bg-indigo-50/50 border border-indigo-100 p-4">
                        <div className="mb-2">
            <span
                className="text-[10px] font-bold uppercase tracking-wider text-indigo-600 bg-indigo-100/80 px-2 py-0.5 rounded">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 relative">
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-4">
                        {[
                            ...
                            <div key={i}
                                ...
                                <p className="text-[10px] text-slate-400 mb-0.5 truncate">{m.label}</p>
                                <p className="text-sm font-bold text-slate-900 leading-tight truncate">{m.value}</p>
                            </div>
                        ))}
                    </div>

                    <div className="rounded-lg bg-amber-50 border border-amber-200 px-3 py-2">
                        <p className="text-[10px] font-semibold uppercase tracking-wide text-amber-600 mb-0.5">Budget
                            Health</p>
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm flex flex-col">
                    ...
                    <div className="rounded-lg bg-slate-900 text-white px-4 py-3 mb-4">
                        <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400 mb-1">Top Action
                            Item</p>
