...
export function AutomationSidebar({
  ...
  const handleTestRule = async (rule: AutomationRule) => {
    ...
    const toastId = toast.loading(
      `Triggering test for ${rule.recipientEmail}...`,
    );
    ...
    try {
      ...
      console.error(
        "💥 [Frontend] Critical error during manual trigger:",
        error,
      );
  ...
  const handleSaveSchedule = async () => {
    ...
    setIsSaving(true);
    const toastId = toast.loading(
      editingRuleId ? "Updating schedule..." : "Saving automation rule...",
    );
    ...
    try {
      ...
      if (result.success) {
        toast.success(editingRuleId ? "Schedule updated" : "Schedule created", {
          id: toastId,
        });
        resetForm();
      } else {
            ...
            <div className="space-y-3">
              {rules.length > 0 ? (
                rules.map((rule) => (
                  <div
                    key={rule.id}
                    className={`p-4 border rounded-xl relative group transition-all ${editingRuleId === rule.id ? "border-blue-500 bg-blue-50/30" : "border-slate-200 hover:border-blue-200 hover:bg-blue-50/20"}`}
                  >
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-xs font-medium text-slate-500 italic">
                        {rule.frequency === "MONTHLY"
                          ? `On the ${rule.dayOfMonth}${rule.dayOfMonth === 1 ? "st" : rule.dayOfMonth === 2 ? "nd" : rule.dayOfMonth === 3 ? "rd" : "th"}`
                          : `Every Monday`}
                      </span>
                    </div>
            ...
            <div className="space-y-5">
              ...
              <div className="p-4 rounded-xl bg-blue-50 border border-blue-100">
                ...
                <p className="text-xs text-blue-700 leading-relaxed opacity-80 mb-3">
                  Gemini will interpret performance shifts and draft a
                  personalized intro.
                </p>
