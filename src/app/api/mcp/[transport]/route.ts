...
import {
    ...
    getAccountTargetsAction, getConcentrationReportAction, getAccountAnomaliesAction, getCampaignDetailsAction,
    getSearchTermInsightsAction, getHistoricalComparisonAction, listAccountsAction
    ...
    (server) => {
        ...
        server.registerTool(
            "get_historical_comparison",
            {
                ...
                inputSchema: {
                    accountId: z.number().describe("The internal database ID of the ad account"),
                    startDate: z.string().describe("Start date of the CURRENT period in YYYY-MM-DD format"),
                    endDate: z.string().describe("End date of the CURRENT period in YYYY-MM-DD format"),
                },
            },
            async ({ accountId, startDate, endDate }) => {
                const result = await getHistoricalComparisonAction(accountId, startDate, endDate);
        ...
        server.registerTool(
            ...
            async ({ accountId, startDate, endDate, limit }) => {
                const result = await getSearchTermInsightsAction(accountId, startDate, endDate, limit);
