...
import { fetchTopClientLandingPage } from "@/lib/google-ads";
import { fetchTopNonBrandedSearchTerm } from "@/lib/google-ads";
...
// ============================================================================
export async function getAutoTargetAction(googleAccountId: string, accountName: string) {
    try {
        console.log(`[Auto-Target] Fetching top non-branded search term for ${accountName}...`);
        const term = await fetchTopNonBrandedSearchTerm(googleAccountId, accountName);
...
async function scrapeAndCompressLandingPage(targetUrl: string) {
    try {
        ...
        $('script, style, noscript, svg, img, nav, footer, iframe, meta, link, header').remove();

        let highValueHtml = '';
        $('h1, h2, h3').each((i, el) => { highValueHtml += $.html(el) + '<br/>'; });
        $('a, button, .btn').each((i, el) => { highValueHtml += $.html(el) + '<br/>'; });
        $('ul, ol').each((i, el) => { highValueHtml += $.html(el) + '<br/>'; });
        $('p').slice(0, 10).each((i, el) => { highValueHtml += $.html(el) + '<br/>'; });

        const turndownService = new TurndownService();
        const cleanMarkdown = turndownService.turndown(highValueHtml || $.html('body'));
...
// ============================================================================
async function getLiveCompetitorsFromGoogleDirect(searchTerm: string, clientDomain: string) {
    console.log(`\n[SERP Sniper] ==========================================`);
    ...
    try {
        ...
        if (!response.ok) {
            console.error(`[SERP Sniper] ❌ HTTP Error: ${response.status} ${response.statusText}`);
            throw new Error(`Scrape.do returned HTTP ${response.status}`);
        }

        const html = await response.text();
        console.log(`[SERP Sniper] ✅ Received HTML response. Size: ${(html.length / 1024).toFixed(2)} KB`);
        ...
        if (html.length < 50000) {
            console.warn(`[SERP Sniper] ⚠️ WARNING: HTML size is unusually small. Scrape.do might have hit a captcha.`);
        }
        ...
        console.log(`[SERP Sniper] 🔍 Found ${adLinks.length} total '/aclk?' tracking links in the DOM.`);
        ...
        adLinks.each((i, el) => {
            ...
            let fullUrl = href.startsWith('http') ? href : `https://www.google.com.au${href}`;

            // Clean up the text for logging and comparison
            const visibleText = $(el).text().toLowerCase().replace(/\s+/g, ' ').trim();

            console.log(`\n[SERP Sniper] --- Inspecting Ad Link ${i + 1} ---`);
            console.log(`[SERP Sniper] 📝 Visible Text: "${visibleText.substring(0, 60)}${visibleText.length > 60 ? '...' : ''}"`);

            // Rejection 1: Is it our own client?
            if (visibleText.includes(clientDomain.toLowerCase())) {
                console.log(`[SERP Sniper] 🛑 REJECTED: Matches client domain (${clientDomain})`);
                return;
            }

            // Rejection 2: Is it a duplicate link? (Google puts multiple links per ad block)
            if (competitorUrls.includes(fullUrl)) {
                console.log(`[SERP Sniper] 🛑 REJECTED: Duplicate URL already in array`);
                return;
            }
        ...
        if (competitorUrls.length === 0) {
            console.log(`[SERP Sniper] 📉 RESULT: No valid competitor ads found for "${searchTerm}".`);
        } else {
            console.log(`[SERP Sniper] 📈 RESULT: Successfully extracted ${competitorUrls.length} competitor URLs.`);
        }

        return competitorUrls;

    } catch (error: any) {
        console.error(`\n[SERP Sniper Error] Failed to fetch competitors for "${searchTerm}":`, error.message || error);
        throw new Error(`Competitor discovery failed: ${error.message || 'Unknown network error'}`);
    }
}
...
// ============================================================================
async function getLiveCompetitorsFromSerper(searchTerm: string, clientDomain: string) {
    ...
    try {
        ...
        console.error(`[Serper Error] Failed to fetch competitors for "${searchTerm}":`, error.message || error);
...
// ============================================================================
export async function generateThreatMatrixAction(adAccountId: number, searchTerm: string) {
    try {
        ...
        if (!clientUrl) {
            console.log(`[Threat Matrix] No websiteUrl in DB. Fetching top landing page from Google Ads API...`);
            const topUrl = await fetchTopClientLandingPage(account.googleAccountId);

            if (!topUrl) {
                throw new Error("Could not detect an active landing page for this client from Google Ads.");
            }
            clientUrl = topUrl;
        ...
        const competitorUrls = await getLiveCompetitorsFromGoogleDirect(searchTerm, clientDomain);

        if (competitorUrls.length === 0) {
            return { success: false, error: "No competitor ads found running for this keyword right now." };
        }

        // STEP 4: Scrape Client & Competitors in Parallel
        console.log(`[Threat Matrix] Scraping client and ${competitorUrls.length} competitors...`);
        ...
        const [savedAudit] = await db.insert(threatMatrixAudits).values({
