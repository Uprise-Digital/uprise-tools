...
export const clients = pgTable(
  "clients",
  {
    ...
    metaEnabled: boolean("meta_enabled").default(true).notNull(),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
  },
  () => [
...
export const weeklyClientReportSettings = pgTable(
  "weekly_client_report_settings",
  {
    ...
    sendTime: varchar("send_time", { length: 5 }).notNull().default("08:00"),
    isActive: boolean("is_active").default(true).notNull(),
    includeRiskWatchlist: boolean("include_risk_watchlist")
      .default(true)
      .notNull(),
    includePerformanceMetrics: boolean("include_performance_metrics")
      .default(true)
      .notNull(),
    includeSentimentPrompt: boolean("include_sentiment_prompt")
      .default(true)
      .notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
  },
...
export const clientReportSettings = pgTable("client_report_settings", {
  id: serial("id").primaryKey(),
  organizationId: text("organization_id").notNull().default("default-org"),
  isGloballyActive: boolean("is_globally_active").default(true).notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
}).enableRLS();
