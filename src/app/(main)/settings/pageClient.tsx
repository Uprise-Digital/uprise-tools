...
function getPrettySyncError(rawError: string) {
  ...
  if (
    err.includes("caller does not have permission") ||
    err.includes("permission_denied") ||
    err.includes("customer_not_enabled")
  ) {
  ...
  if (
    ...
    err.includes("request contains an invalid argument")
  ) {
...
export default function SettingsClient({
  ...
  // Onboarding Configurator States
  ...
  const handleManualSync = async () => {
    ...
    try {
      ...
      if (res.success) {
        toast.success(
          "Portfolio sync triggered successfully! Running in the background.",
          { id: toastId },
        );
        window.location.reload();
      } else {
        toast.error(res.error || "Failed to trigger portfolio sync", {
          id: toastId,
        });
      }
    } catch (err: any) {
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="p-3 bg-slate-50 border border-slate-100 rounded-lg text-xs">
                          ...
                          <span className="font-semibold text-slate-800 block mt-1">
                            ...
                            ad accounts synced under this connection.
                          </span>
                        </div>
                      ...
                      <div className="space-y-4 text-center py-6">
                        ...
                        <div className="max-w-sm mx-auto space-y-1">
                          ...
                          <p className="text-xs text-slate-500">
                            Please link a Google Ads account to start monitoring
                            and triaging campaign portfolios.
                          </p>
                        </div>
                        <Button
                          asChild
                  ...
                  <CardContent className="p-6 space-y-4">
                    ...
                    <div className="flex items-start gap-2.5 p-3 bg-slate-50 border border-slate-100 rounded-xl mt-3 select-none">
                      <input
                        ...
                        onChange={(e) =>
                          setAllowDomainAutoJoin(e.target.checked)
                        }
              ...
              <div className="space-y-6">
                <Card className="py-0 border-slate-200 shadow-sm overflow-hidden">
                  ...
                  <CardContent className="p-5 text-xs text-slate-500 leading-relaxed space-y-3">
                    <p>
                      <strong>Disconnecting Google Ads</strong> will stop all
                      background triages and landing page audits. You can choose
                      to retain imported accounts and histories or erase them
                      entirely.
                    </p>
                    <p>
                      Use <strong>Add or Remove Accounts</strong> to link new
                      client accounts under your connected manager account or
                      hide accounts you no longer want to analyze.
                    </p>
                  </CardContent>
                </Card>
              </div>
            ...
            <DialogContent className="max-w-md bg-white border rounded-xl p-6">
              <DialogHeader>
                ...
                <DialogDescription className="text-xs text-slate-500 mt-2">
                  This will remove Uprise Digital's credentials access to Google
                  Ads. All background campaign syncing will stop immediately.
                </DialogDescription>
              </DialogHeader>
                ...
                <label className="flex items-start gap-3 p-3 bg-slate-50 border rounded-lg cursor-pointer hover:bg-slate-100/50 transition-colors">
                  ...
                  <div className="text-xs">
                    ...
                    <span className="text-slate-500 mt-0.5 block">
                      Erases all metric breakdowns, campaign landing pages,
                      audit logs, and keywords permanently.
                    </span>
                  </div>
                </label>
              </div>
            ...
            <DialogContent className="max-w-4xl bg-slate-950 text-white border border-slate-800 rounded-xl p-6 overflow-hidden flex flex-col max-h-[85vh]">
              <DialogHeader className="border-b border-slate-800 pb-4">
                ...
                <DialogDescription className="text-xs text-slate-400">
                  Select which sub-accounts to track in Uprise Digital.
                  Unchecked accounts will be set to inactive and hidden.
                </DialogDescription>
              </DialogHeader>
                        ...
                        <tbody className="divide-y divide-slate-850">
                          {previewAccounts.filter((acc) => {
                            ...
                            previewAccounts
                              ...
                              .map((acc) => {
                                const isChecked = selectedAccountIds.has(
                                  acc.id,
                                );
                                return (
                                  <tr
              ...
              <div className="lg:col-span-2 space-y-6">
                <Card className="py-0 border-slate-200 shadow-sm overflow-hidden">
                  <CardHeader className="bg-slate-50 border-b border-slate-100 p-5">
                    ...
                    <CardDescription className="text-xs">
                      Manage global alert logic thresholds. These values serve
                      as agency-wide defaults unless overridden at the
                      individual client account level.
                    </CardDescription>
                  </CardHeader>
                ...
                <Card className="border-slate-200 shadow-sm overflow-hidden">
                  <CardHeader className="bg-slate-50 border-b border-slate-100 p-5 flex flex-row items-center justify-between">
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent
                        side="left"
                        className="bg-slate-900 text-white text-[10px] p-2 rounded-lg border border-slate-850"
                      >
                        Sync Portfolio Now
                      </TooltipContent>
                    </Tooltip>
                        ...
                        return (
                          ...
                          >
                            ...
                            <TableCell>
                              <Badge
                                variant={
                                  getActionBadgeVariant(log.action) as any
                                }
            ...
            <Card className="border-slate-200 shadow-sm overflow-hidden animate-in fade-in duration-200">
              <CardHeader className="bg-slate-50 border-b border-slate-100 p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  ...
                  <CardDescription className="text-xs">
                    Log of outgoing Resend email dispatches for briefings,
                    automated schedules, and on-demand campaigns.
                  </CardDescription>
                </div>
                {/* SEARCH AND FILTERS */}
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <div className="p-3 bg-slate-50 border rounded-lg flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-slate-500 font-mono">
                        Actor ID:{" "}
                        {selectedAuditLog.actorId || "unknown_system_id"}
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-4 pt-2 font-sans text-xs">
                  <div className="grid grid-cols-2 gap-4 border-b pb-4">
                    <div className="space-y-0.5">
                      ...
                      <span className="font-semibold text-slate-800">
                        {new Date(selectedEmailLog.sentAt).toLocaleString(
                          "en-AU",
                          {
                            weekday: "long",
                            year: "numeric",
