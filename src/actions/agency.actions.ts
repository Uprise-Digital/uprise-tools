...
import {getCurrentPeriodDateClause, getManagementAccessToken, getPreviousPeriodDateClause} from "@/lib/google-ads";
...
export async function getHistoricalComparisonAction(
    ...
    try {
        ...
        if (!account) {
            return { success: false, error: `No account found with ID ${accountId}.` };
        }
        ...
        if (!currentRes.success || !currentRes.data) {
            return { success: false, error: "Failed to fetch current period metrics." };
        }
        ...
        if (!prevStart || !prevEnd) {
            return { success: false, error: "Could not calculate previous period dates." };
        }
    ...
    try {
        ...
        const terms = results.map((row: any) => {
            ...
            return {
                ...
                cpa: conversions > 0 ? parseFloat((spend / conversions).toFixed(2)) : null,
    ...
    try {
        ...
        if (baselineRows.length === 0) {
            return {
                ...
                data: {
                    account: { id: account.id, name: account.name },
                    message: "Insufficient historical data to compute anomalies. Need at least one full prior period in adPerformanceDaily.",
                    anomalies: [],
                },
            };
        }
        ...
        const check = (
            ...
            const isBad = higherIsBad ? changePct > threshold : changePct < -threshold;
        ...
        check("daily_avg_spend",       recent.daily_avg_spend,       baseline.daily_avg_spend,       30, false);
        check("daily_avg_conversions",  recent.daily_avg_conversions,  baseline.daily_avg_conversions,  25, false); // decrease is bad
        ...
        return {
            ...
            data: {
                account: { id: account.id, name: account.name },
                windows: {
                    recent: { startDate: recentStartStr, endDate: todayStr, metrics: recent },
                    baseline: { startDate: baselineStartStr, endDate: baselineEndStr, metrics: baseline },
                },
                anomalies: anomalies.sort((a, b) => Math.abs(b.change_pct) - Math.abs(a.change_pct)),
            },
        };
    } catch (error: any) {
        return { success: false, error: error.message };
...
export async function getConcentrationReportAction(
    ...
    try {
        // Current period
        const currentRes = await getAgencyPortfolioMetricsAction(startDate, endDate);
        if (!currentRes.success || !currentRes.data) {
        ...
        const previousRes = prevStart && prevEnd
        ...
        const computeConcentration = (breakdown: any[], totalSpend: number) => {
            ...
            const whales = breakdown
                ...
                .map(acc => ({
                    ...
                    spend_share_pct: parseFloat(((acc.spend / totalSpend) * 100).toFixed(1)),
                }));
            ...
            return {
                ...
                hhi_interpretation: hhi > 2500 ? "Highly Concentrated" : hhi > 1500 ? "Moderately Concentrated" : "Diversified",
                whales,
        ...
        const currentConcentration = computeConcentration(currentBreakdown, currentTotalSpend);
        ...
        return {
            ...
            data: {
                ...
                previous: previousConcentration
                    ? { period: { startDate: prevStart, endDate: prevEnd }, ...previousConcentration }
                    : null,
                hhi_trend: previousConcentration
                    ? parseFloat((currentConcentration.hhi - previousConcentration.hhi).toFixed(0))
                    : null,
...
export async function getAccountTargetsAction(accountId: number) {
    try {
        ...
        return {
            success: true,
            data: {
                account: { id: account.id, name: account.name, currencyCode: account.currencyCode },
                targets,
                warning: !hasTargets
