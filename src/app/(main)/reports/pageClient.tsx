...
export default function AgencyReportsClient() {
    ...
    useEffect(() => {
        ...
        fetchPortfolioData(isMounted).then(() => {
            // After data loads, silently check if an AI report is cached for these dates
            if (isMounted) {
    ...
    const fetchGodModeAi = async (forceRefresh: boolean, isMounted = true) => {
        ...
        try {
            // If we are forcing a refresh, we MUST pass the portfolio data so it has something to analyze
    ...
    // Formatters
    const fCur = (v: number) => new Intl.NumberFormat('en-US', {
        style: 'currency',
        currency: 'AUD'
    }).format(isNaN(v) ? 0 : v);
        ...
        <div className="space-y-8 p-4 md:p-8 max-w-[1600px] mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-3">
                    <Button onClick={handleSyncPortfolio} disabled={isSyncing || loadingData} variant="outline"
                            className="w-full sm:w-auto bg-white">
                        {isSyncing ? <><Loader2 className="h-4 w-4 mr-2 animate-spin text-blue-600"/> Syncing...</> : <>
                            <CloudDownload className="h-4 w-4 mr-2 text-blue-600"/> Sync Portfolio</>}
                    </Button>

                    <div
                        className="flex items-center bg-white rounded-full border border-slate-200 shadow-sm px-4 py-1.5 gap-2">
                ...
                {[
                    {
                        ...
                        val: portfolio?.agencyTotals.activeAccountsCount,
                        icon: Users,
                    ...
                    {
                        label: "Blended Spend",
                        val: fCur(portfolio?.agencyTotals.spend),
                        icon: DollarSign,
                        color: "text-blue-600",
                        bg: "bg-blue-50"
                    },
                    {
                        ...
                        val: fNum(portfolio?.agencyTotals.conversions),
                        icon: Target,
                    ...
                    {
                        label: "Blended CPA",
                        val: fCur(portfolio?.agencyTotals.cpa),
                        icon: Activity,
                        color: "text-rose-600",
                        bg: "bg-rose-50"
                    },
            ...
            <Card className="border-slate-300 shadow-md bg-slate-950 text-slate-100 overflow-hidden relative">
                ...
                {isAiRefreshing && <div
                    className="absolute inset-0 bg-slate-950/50 backdrop-blur-[1px] z-10 flex items-center justify-center">
                    <Loader2 className="h-8 w-8 animate-spin text-blue-500"/></div>}

                <CardHeader
                    className="border-b border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between py-4 gap-3">
                    ...
                    <div className="flex items-center gap-3">
                        {generatedAt && (
                            <div
                                className="flex items-center gap-1.5 text-xs text-slate-400 bg-slate-900 px-3 py-1.5 rounded-md border border-slate-800">
                        ...
                        >
                            {isAiRefreshing ? <><Loader2 className="h-4 w-4 mr-2 animate-spin"/> Analyzing...</> : <>
                ...
                <CardContent className="p-0">
                    {isAiLoading ? (
                        <div className="p-12 flex flex-col items-center justify-center text-slate-400 gap-3">
                            ...
                            <p className="text-sm">Scanning entire portfolio for churn risks and performance
                                fires...</p>
                        </div>
                    ) : insights ? (
                        <div
                            className="grid grid-cols-1 lg:grid-cols-3 divide-y lg:divide-y-0 lg:divide-x divide-slate-800">
                            {/* Macro & Growth */}
                            <div className="p-6 lg:col-span-1 space-y-6">
                                <div>
                                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Macro
                                        Summary</h4>
                                    <p className="text-sm leading-relaxed text-slate-300">{insights.macro_summary}</p>
                                </div>
                                <div>
                                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Blended
                                        Efficiency</h4>
                            ...
                            <div className="p-6 lg:col-span-2 bg-slate-900/50">
                                ...
                                {insights.critical_fires?.length === 0 ? (
                                    <div
                                        className="bg-emerald-950/30 border border-emerald-900/50 rounded-lg p-4 text-emerald-400 text-sm flex items-center gap-2">
                                        <TrendingUp className="h-4 w-4"/> All accounts are operating within acceptable
                                        parameters. No critical churn risks detected.
                                    </div>
                                ) : (
                                        ...
                                        {insights.critical_fires?.map((fire: any, i: number) => (
                                            <div key={i}
                                                ...
                                                <div className="flex justify-between items-start mb-2">
                                                    ...
                                                    <span
                                                        className="text-[10px] bg-red-500 text-white px-2 py-0.5 rounded uppercase font-bold">{fire.severity}</span>
                                                </div>
                                                <p className="text-sm text-slate-300 mb-2"><strong
                                                    className="text-slate-100">Issue:</strong> {fire.the_problem}</p>
                                                <p className="text-sm text-amber-200/80"><strong
                                                    className="text-amber-400">Fix:</strong> {fire.recommended_action}
                                                </p>
                                            </div>
                                        ))}
                                    </div>
            ...
            <Card className="shadow-sm border-slate-200">
                <CardHeader
                    className="border-b border-slate-100 bg-slate-50/50 flex flex-row justify-between items-center py-4">
                            ...
                            {portfolio?.accountBreakdown.map((acc: any) => (
                                <TableRow key={acc.accountId}
                                    ...
                                    <TableCell>
                                        ...
                                        <div
                                            className="text-[10px] text-slate-500 font-mono mt-0.5">{acc.googleAccountId}</div>
                                    </TableCell>
                                    <TableCell className="text-right font-mono">{fCur(acc.spend)}</TableCell>
                                    <TableCell
                                        className="text-right font-mono font-bold text-emerald-600">{fNum(acc.conversions)}</TableCell>
                                    <TableCell className="text-right font-mono">
                                        <span
                                            className={acc.cpa > (portfolio.agencyTotals.cpa * 1.5) ? "text-red-600 font-bold bg-red-50 px-2 py-1 rounded" : ""}>
                                            {fCur(acc.cpa)}
                                        </span>
                                    </TableCell>
                                    <TableCell className="text-right font-mono">
                                        <span
                                            className={acc.ctr > 0 && acc.ctr < 3 ? "text-amber-600 font-bold bg-amber-50 px-2 py-1 rounded" : "text-slate-500"}>
                                            {fPct(acc.ctr)}
                                        </span>
                                    </TableCell>
                                    <TableCell
                                        className="text-right font-mono text-slate-500">{fCur(acc.cpc)}</TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                    </Table>
