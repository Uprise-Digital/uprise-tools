...
function getPrettySyncError(rawError: string) {
  if (!rawError) return "";
  if (rawError.includes("DEVELOPER_TOKEN_PROHIBITED")) {
    return "Developer token is prohibited on this manager account. Verify connected MCC settings.";
  }
  if (rawError.includes("USER_PERMISSION_DENIED")) {
    return "Linked user does not have permission to sync this client account.";
  }
  if (rawError.includes("OAUTH_TOKEN_EXPIRED")) {
    return "Your Google Ads login has expired. Disconnect and re-link your Google account.";
  }
  return rawError;
}

export function TriageTab({ initialDefaults, accounts }: TriageTabProps) {
          ...
          <form onSubmit={handleSave}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <p className="text-[10px] text-slate-400">
                      Trigger alert if an account spends more than this with low
                      conversions.
                    </p>
                  </div>
                  <div className="space-y-1.5">
            ...
            <CardFooter className="border-t border-slate-100 p-5 bg-slate-50 flex items-center justify-between">
              <div className="text-xs text-slate-500">
                {hasChanges ? (
                  <span className="text-amber-650 font-medium flex items-center gap-1">
                          ...
                          <div className="text-[10px] text-rose-600 bg-rose-50/50 p-2.5 rounded-lg border border-rose-100/50 flex items-start gap-2 leading-relaxed shadow-sm">
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <button
                                  type="button"
                                  className="p-1 -m-1 hover:bg-rose-100 rounded-md transition-colors text-rose-455 hover:text-rose-600 shrink-0 cursor-pointer"
                                >
                              ...
                              >
                                <p className="font-bold text-slate-450 mb-1 border-b border-slate-800 pb-1">
