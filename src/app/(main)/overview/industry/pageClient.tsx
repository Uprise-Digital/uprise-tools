...
export default function IndustryAnalyticsClient() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getIndustryPortfolioMetricsAction(
        startDate,
        endDate,
        platformFilter,
      );
  ...
  const filteredAccounts = useMemo(() => {
    ...
    return data.allAccounts.filter((acc) => {
      ...
      const matchesSearch =
        ...
        acc.metaAccountId?.includes(accountSearch) ||
        acc.subNiche?.toLowerCase().includes(q);
  ...
  return (
    <div className="space-y-8 p-4 md:p-8 max-w-[1600px] mx-auto">
        ...
        <div className="flex items-center gap-2 max-w-full overflow-x-auto pb-1 sm:pb-0 sm:overflow-visible shrink-0">
          ...
          <div className="flex items-center bg-slate-200/80 p-0.5 rounded-lg shrink-0">
            <button
              type="button"
              onClick={() => {
                if (platformFilter !== "all") setPlatformFilter("all");
              }}
              disabled={loading}
      ...
      {/* ── 2. DATA CONTENT CONTAINER WITH LOADING OVERLAY ── */}
      <div className="relative min-h-[500px]">
        {loading && (
          <div className="absolute inset-0 z-30 flex flex-col items-center justify-start pt-24 bg-white/60 backdrop-blur-[1.5px] rounded-2xl pointer-events-none transition-all duration-200">
            <div className="sticky top-28 flex items-center gap-2.5 px-4 py-2.5 bg-slate-900/90 backdrop-blur text-white rounded-full shadow-2xl border border-slate-800 text-xs font-semibold animate-in fade-in zoom-in-95 duration-150">
        ...
        >
              ...
              <CardContent className="p-4 flex items-center justify-between">
                <div className="space-y-0.5 min-w-0 pr-2">
                  ...
                  <p className="text-[11px] font-bold text-slate-500">
                    ...
                    (
                    {topSpendingGroup
                      ? fPct(topSpendingGroup.spendSharePct)
                      : "0%"}
                    )
                  </p>
                </div>
              ...
              <CardContent className="p-4 flex items-center justify-between">
                <div className="space-y-0.5 min-w-0 pr-2">
                  ...
                  <p className="text-base font-black text-emerald-900 truncate">
                    {bestEfficiencyGroup
                      ? bestEfficiencyGroup.shortLabel
                      : "N/A"}
                  </p>
              ...
              <CardContent className="p-4 flex items-center justify-between">
                <div className="space-y-0.5">
                  ...
                  <p className="text-[11px] text-slate-500 font-semibold">
                    {fNum(data?.agencyTotals?.totalConversions || 0)} Total
                    Leads
                  </p>
                </div>
          ...
          <div className="space-y-3">
            <h3 className="text-xs font-extrabold uppercase tracking-widest text-slate-400 flex items-center gap-1.5">
              <Scale className="w-3.5 h-3.5 text-indigo-500" /> Vertical
                          ...
                          <Tooltip
                            content={({ active, payload }) => {
                              if (active && payload && payload.length) {
                                const p = payload[0];
                                const total =
                                  data?.agencyTotals?.totalSpend || 1;
                                const pct = (
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {data?.industryGroups
                        ...
                        .map((group) => {
                          const isSelected =
                            selectedVertical === group.industry;
                          return (
                            <TableRow
                              key={group.industry}
                              onClick={() =>
                      ...
                      {(!data?.industryGroups ||
                        ...
                        <TableRow>
                          ...
                          >
                            No accounts assigned yet. Click "Auto-Classify with
                            AI" above to automatically tag accounts.
                          </TableCell>
                        </TableRow>
                      )}
                    </TableBody>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div>
                <h3 className="text-xs font-extrabold uppercase tracking-widest text-slate-400 flex items-center gap-1.5">
                  ...
                  Intra-Industry Peer Rankings & Efficiency
                </h3>
                  ...
                  <TableBody>
                    {filteredAccounts.map((acc) => {
                      ...
                      return (
                        <TableRow
                          key={
                            acc.key || `${acc.primaryPlatform}-${acc.accountId}`
                          }
                          ...
                          <TableCell className="py-3 pl-5">
                            <div>
                              {acc.googleId && platformFilter !== "meta" ? (
                                ...
                                <a
                                  href={`https://adsmanager.facebook.com/adsmanager/manage/campaigns?act=${acc.metaAccountId}`}
                                  target="_blank"
                                  rel="noreferrer"
                                  className="text-xs font-bold text-slate-900 hover:text-sky-600 transition-colors flex items-center gap-1 group"
                                >
                                  <span>{acc.name}</span>
                          ...
                          <TableCell className="text-center py-3 pr-5">
                            <div className="flex items-center justify-center gap-1.5">
                              ...
                              {acc.googleId && platformFilter !== "meta" ? (
                                ...
                                >
                                  <Link href={`/accounts/${acc.googleId}`}>
