...
import { db } from "@/db";
import { emailLogs, organization, organizationEmailTemplates } from "@/db/schema";
...
<p>Feel free to reach out if you have any questions!</p>`,
    variables: [
      { name: "primary_contact_name", description: "Primary contact full name" },
...
<p style="font-size: 11px; color: #64748b; margin-top: 20px;">Queries or issues can be directed to support: {{support_email}}</p>`,
    variables: [
      ...
      { name: "briefing_content", description: "HTML summary of key agency metrics and alerts" },
      { name: "agency_name", description: "Your agency display name" },
...
<p>Thank you for partnering with {{agency_name}}.</p>`,
    variables: [
      ...
      { name: "report_url", description: "Link to interactive performance report" },
      { name: "agency_name", description: "Your agency display name" },
    ],
  },
  pipeline_digest: {
    ...
    category: "team",
    defaultSubject: "⚠️ Pipeline Sales Digest: {{stalled_count}} Stalled Deals Requires Attention",
...
<p><a href="{{pipeline_url}}">View Pipeline Dashboard</a></p>`,
    variables: [
      { name: "stalled_count", description: "Number of stalled deal opportunities" },
      { name: "pipeline_content", description: "HTML list of stalled leads" },
      { name: "pipeline_url", description: "Direct URL to pipeline dashboard" },
      { name: "agency_name", description: "Your agency display name" },
    ],
  },
  team_invite: {
...
<p><a href="{{invite_url}}" style="display: inline-block; padding: 10px 18px; background-color: #4f46e5; color: #ffffff; text-decoration: none; border-radius: 6px; font-weight: bold;">Accept Invitation & Join</a></p>`,
    variables: [
      { name: "role", description: "Assigned member role (Admin, Member, etc.)" },
...
export async function sendSystemEmail(params: SendSystemEmailParams) {
  ...
  if (!templateSubject || !templateHtml) {
    ...
    templateSubject = customSubject || customDbTemplate?.subject || defaultDef?.defaultSubject || "Notification";
    templateHtml = customHtml || customDbTemplate?.bodyHtml || defaultDef?.defaultHtml || "<p>Notification email.</p>";
  }
...
${websiteUrl ? `<p style="font-size: 12px; color: #64748b; margin: 0;"><a href="${websiteUrl}" style="color: #4f46e5; text-decoration: none;">${websiteUrl}</a></p>` : ""}`;
  ...
  try {
    ...
    const fromAddress = process.env.SENDER_EMAIL || "Uprise Digital <reports@uprisedigital.com.au>";
    ...
    console.error(`[Email Service Error] Failed to send email (${templateKey}):`, err);
