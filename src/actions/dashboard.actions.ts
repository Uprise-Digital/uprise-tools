...
export async function deleteTeamMember(targetUserId: string) {
  ...
  try {
    ...
    // SUCCESS LOGGING
    await logAction(session.user.id, "DELETE_USER", "user", targetUserId, {
 ...
 */
async function syncGoogleAdsDataToDb(
  adAccountId: number,
  ...
  console.log(
    `[JIT Sync] Fetching Google Ads data for ${googleAccountId} from ${startDate} to ${endDate}`,
  );

  const rawData = await fetchDailyCampaignData(
    googleAccountId,
    startDate,
    endDate,
  );
  ...
  await db
    ...
    .onConflictDoUpdate({
      target: [
        ...
        adPerformanceDaily.date,
        adPerformanceDaily.campaignId,
      ],
      set: {
  ...
  console.log(
    `[JIT Sync] Successfully upserted ${insertPayload.length} daily campaign records.`,
  );
}
...
export async function getDashboardMetricsAction(
  ...
  try {
    ...
    const rawTotals = await db
      ...
      .then(
        (res) =>
          res[0] || { spend: 0, clicks: 0, impressions: 0, conversions: 0 },
      );
