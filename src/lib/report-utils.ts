...
export function transformAdsData(
  ...
  const prev = lastMonth
    ? {
        ...
        ctr:
          Number(lastMonth.impressions) > 0
            ? (Number(lastMonth.clicks) / Number(lastMonth.impressions)) * 100
            : 0,
        cpc:
          Number(lastMonth.clicks) > 0
            ? Number(lastMonth.costMicros) /
              1_000_000 /
              Number(lastMonth.clicks)
            : 0,
        cpa:
          Number(lastMonth.conversions) > 0
            ? Number(lastMonth.costMicros) /
              1_000_000 /
              Number(lastMonth.conversions)
            : 0,
      }
    : null;
