...
const navItems: NavItem[] = [
  {
    ...
    subItems: [
      { href: "/overview", label: "God View", icon: Activity },
      {
            ...
            <div key={item.href} className="flex flex-col gap-1">
              ...
              >
                <div className="flex items-center gap-2.5">
                  <Icon className="h-4.5 w-4.5 shrink-0 text-indigo-400" />
                  <span className="truncate">{item.label}</span>
                </div>
                <ChevronDown
                  className={cn(
                    "h-3.5 w-3.5 text-slate-500 transition-transform duration-200",
                    isExpanded ? "rotate-0 text-slate-300" : "-rotate-90",
                  )}
                />
              </button>

              {isExpanded && (
                <div className="pl-3.5 pr-1 flex flex-col gap-1 border-l border-slate-800/80 ml-4 my-0.5">
                  {item.subItems.map((sub) => {
                    ...
                    return (
                      <Link
                        ...
                        className={cn(
                          "flex items-center gap-2 px-3 py-2 rounded-lg transition-all text-[11px] font-semibold tracking-wide",
                          isSubActive
