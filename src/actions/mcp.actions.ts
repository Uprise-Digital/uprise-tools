...
export async function getMcpSettingsAction() {
    ...
    if (!settings) {
        const [newSettings] = await db.insert(mcpSettings).values({
            agencyId,
...
export async function rollMcpApiKeyAction() {
    try {
        ...
        const [updated] = await db.update(mcpSettings)
 ...
 */
export async function updateMcpToolsAction(toolsConfig: Record<string, boolean>) {
    try {
        const agencyId = await getAgencyId();

        await db.update(mcpSettings)
